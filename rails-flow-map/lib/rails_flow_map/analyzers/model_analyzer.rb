...
module RailsFlowMap
  class ModelAnalyzer
    ...
    def analyze_model_file(file)
      ...
      Rails.logger&.warn "Error analyzing #{file}: #{e.message}"
    end
    ...
    class ModelVisitor < Parser::AST::Processor
      ...
      def extract_class_name(node)
        ...
        Rails.logger&.warn "Failed to extract class name: #{e.message}"
        nil
      end
      ...
      def is_active_record_model?(superclass_node)
        ...
        Rails.logger&.warn "Failed to check ActiveRecord model: #{e.message}"
        false
      end
      ...
      def process_association(method_name, args)
        ...
        Rails.logger&.warn "Failed to process association: #{e.message}"
      end
      ...
      def extract_association_name(node)
        ...
        Rails.logger&.warn "Failed to extract association name: #{e.message}"
        nil
      end
